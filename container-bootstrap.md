# UGREEN DXP VXLAN Bootstrap — Docker-only method
...
At minimum, configure dependent services so failed starts are retried (`restart: unless-stopped` or an equivalent UGOS startup mechanism). For deterministic startup and Docker restart ordering, use the [boot-safe systemd method](boot-safe.md) instead.
